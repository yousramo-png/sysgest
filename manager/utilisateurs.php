...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['update_user'])) {
    ...
    if ($user_id && $name && $email && $rolePost) {
        ...
        $updateError = "Tous les champs requis ne sont pas remplis.";
    }
}
    ...
    if (empty($name) || empty($email) || empty($password) || empty($confirm_password) || empty($rolePost)) {
        ...
        if ($statement->fetch()) {
            ...
            try {
                ...
                $registerSuccess = "Utilisateur ajouté avec succès.";
            } catch (PDOException $e) {
                $registerError = "Erreur lors de l'ajout : " . $e->getMessage();
            }
        }
    }
}
...
if ($filterQ !== '') {
    $conditions[] = "(u.name LIKE :q_name OR u.email LIKE :q_email OR u.telephone LIKE :q_phone)";
    $bindParams[':q_name']  = "%{$filterQ}%";
    $bindParams[':q_email'] = "%{$filterQ}%";
    $bindParams[':q_phone'] = "%{$filterQ}%";
}
if ($filterRole !== null) {
...
// TOTAL UTILISATEURS
// --------------------
$sqlCount = "SELECT COUNT(*) FROM users u $where";
